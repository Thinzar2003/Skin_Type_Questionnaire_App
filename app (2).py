...
SKIN_INFO = {
    'Dry': {
        ...
        'characteristics': ['Feels tight after washing','Rough or flaky texture','Small barely-visible pores','Prone to irritation','Fine lines more visible'],
        'routine': ['Cream-based gentle cleanser','Hydrating toner (hyaluronic acid)','Rich moisturizer with ceramides','SPF 30+ sunscreen','Overnight hydrating mask 2x/week'],
    ...
    'Normal': {
        ...
        'characteristics': ['Balanced oil and moisture','Small barely-visible pores','Smooth even texture','Rarely sensitive','Naturally radiant'],
        'routine': ['Gentle gel or foam cleanser','Lightweight toner','Light moisturizer','SPF 30+ sunscreen','Weekly exfoliation'],
    ...
    'Oily': {
        ...
        'characteristics': ['Shiny or greasy appearance','Enlarged visible pores','Prone to blackheads','Makeup slides off','Thick skin texture'],
        'routine': ['Foaming cleanser with salicylic acid','BHA or astringent toner','Oil-free lightweight moisturizer','Mattifying SPF 30+','Niacinamide serum'],
    ...
    'Combination': {
        ...
        'characteristics': ['Oily T-zone','Dry or normal cheeks','Enlarged pores on nose','Occasional T-zone breakouts','Different zones need different care'],
        'routine': ['Gentle balancing cleanser','Balancing toner','Light moisturizer on T-zone, richer on cheeks','Lightweight SPF 30+','Zone-specific treatments'],
...
QUESTIONS = [
    ...
    {'q': 'By midday, how does your skin look?',
     'opts': ['Very shiny all over','Shiny only on T-zone (forehead/nose/chin)','Same as morning','Feels drier and tighter']},
    {'q': 'How often do you experience breakouts?',
     'opts': ['Frequently, all over face','Occasionally, mainly T-zone','Rarely','Almost never, but skin is flaky']},
    ...
    {'q': 'How does your skin react to new products?',
     'opts': ['Often irritated or red','Sometimes breaks out','Rarely reacts','Absorbs quickly, needs more']},
    {'q': 'What best describes your skin texture?',
     'opts': ['Rough, flaky or tight','Smooth some areas, oily others','Smooth and balanced overall','Consistently shiny and greasy']},
    {'q': 'Press a tissue to your face — what happens?',
     'opts': ['Lots of oil all over','Oil mainly from T-zone','Very little oil','Almost nothing, skin is dry']},
]
...
MAPPINGS = [
    ...
    {'Very shiny all over':{'Oily':3},'Shiny only on T-zone (forehead/nose/chin)':{'Combination':3},
...
def analyze_image(pil_img):
    """
    Rule-based image analysis using color and texture features.
    Analyzes skin brightness, uniformity, and texture patterns.
    """
    ...
    if face_detected:
        x, y, w, h = faces[0]
        # Focus on central face region (skin area)
        pad = 10
        region = img_bgr[
            max(0,y+pad):y+h-pad,
            max(0,x+pad):x+w-pad
    ...
    # ── Feature extraction ──────────────────────────────────────────
    # 1. Brightness (V channel in HSV)
    hsv = cv2.cvtColor(region, cv2.COLOR_BGR2HSV)
    brightness = float(np.mean(hsv[:,:,2]))
    brightness_std = float(np.std(hsv[:,:,2]))

    # 2. Saturation (S channel)
    saturation = float(np.mean(hsv[:,:,1]))

    # 3. Texture (Laplacian variance = sharpness/roughness)
    gray_region = cv2.cvtColor(region, cv2.COLOR_BGR2GRAY)
    texture_score = float(cv2.Laplacian(gray_region, cv2.CV_64F).var())

    # 4. Uniformity (low std = uniform = oily/normal, high = dry/combination)
    uniformity = float(np.std(gray_region.astype(float)))

    # 5. T-zone vs cheek brightness difference
    ...
    # High brightness variation → combination or dry
    ...
    if texture_score > 300:
        scores['Dry'] += 2      # rough/flaky
    elif texture_score > 150:
        scores['Combination'] += 1
        scores['Normal'] += 1
    else:
        scores['Oily'] += 2     # smooth/shiny

    # Zone difference → combination
    if zone_diff > 20:
...
with tab1:
    ...
    for i, q in enumerate(QUESTIONS):
        ...
        ans = st.radio('',q['opts'],key=f'q{i}',index=None,label_visibility='collapsed')
        answers.append(ans)
        if ans is None: all_answered = False
        st.markdown('')
    ...
    if btn1 and all_answered:
        ...
        st.success('💡 Go to the **⚖️ Compare Both** tab if you also completed the image analysis!')


# ── TAB 2: Image Analysis ────────────────────────────────────────────
with tab2:
    st.markdown('### Upload a clear face photo')
    st.markdown('*For best results: good lighting, face centered, no heavy makeup*')
    st.divider()
...
with tab3:
    ...
    if not q_done and not img_done:
        st.info('Complete both the **Questionnaire** and **Image Analysis** tabs first, then come back here.')
    elif not q_done:
        ...
        if agree:
            ...
            st.markdown('*The questionnaire is generally more reliable. Consider it as your primary result.*')
        ...
        with col1:
            ...
            color_q = COLORS[skin_q]
            bg_q    = BG[skin_q]
            st.markdown(f'''
            <div class="compare-box" style="border-top:4px solid {color_q};background:{bg_q}">
                <div style="font-size:2.5rem">{EMOJI[skin_q]}</div>
                <h3 style="color:{color_q};margin:0.3rem 0">{skin_q} Skin</h3>
                <p style="color:#666;font-size:0.9rem">Confidence: {conf_q}%</p>
            </div>
            ''', unsafe_allow_html=True)
            st.markdown('**Scores:**')
            for st_name, pct in sorted(pcts_q.items(), key=lambda x:-x[1]):
                st.markdown(f'- {st_name}: {pct}%')

        with col2:
            st.markdown('#### 📸 Image Analysis')
            color_i = COLORS[skin_i]
            bg_i    = BG[skin_i]
            st.markdown(f'''
            <div class="compare-box" style="border-top:4px solid {color_i};background:{bg_i}">
                <div style="font-size:2.5rem">{EMOJI[skin_i]}</div>
                <h3 style="color:{color_i};margin:0.3rem 0">{skin_i} Skin</h3>
                <p style="color:#666;font-size:0.9rem">Confidence: {conf_i}%</p>
            </div>
            ''', unsafe_allow_html=True)
            st.markdown('**Scores:**')
            for st_name, pct in sorted(pcts_i.items(), key=lambda x:-x[1]):
                st.markdown(f'- {st_name}: {pct}%')

        st.divider()

        # Final recommendation
        final = skin_q  # questionnaire is primary
